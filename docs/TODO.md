...
| [#3](https://github.com/drlholloway/sightings/issues/3) ✅ | Classify transistors and diodes for classic circuits (Pedal Builder, done 2026‑09‑07) | Circuit profiles (Fuzz Face Q1/Q2, Tone Bender, Rangemaster, Big Muff, clipping diode pairs) with per‑position rules on hFE, DCA55 hFE, leakage and Vf; "Fits" panel on readings. Role tagging is covered by the reading label plus History search. Still open: pair suggestions (best Q1+Q2 sets from a bin by the gain‑ratio rule). |
...
- **Release polish:** macOS notarization (needs an Apple Developer account); Mac App Store and
  Google Play listings. (Demo mode for reviewers done 2026‑09‑07.)
